  ...
  _friendRequestSuccess = response => {
    ...
    let userList = this.state.userList.filter(item => {
      if (item.id == response.profile.basic.id) {
        ...
        else item.friends_meta.has_sent_friend_request_to_this_profile = false;
      }
      return item;
    });
    ...
    return (
      <View>
        <RNButton
          ...
          icon={
            <Icon name="user-plus" size={18} color={Colors.yellowDark}></Icon>
          }
          onPress={() => {
            this._send(item.id);
          }}
